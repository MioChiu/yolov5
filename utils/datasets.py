...
def create_test_dataloader(path, imgsz, batch_size, stride, opt, hyp=None, augment=False, cache=False, pad=0.0, rect=False,
                      rank=-1, world_size=1, workers=8, image_weights=False, quad=False):
    # Make sure only the first process in DDP process the dataset first, and the following others can use the cache
    assert batch_size == 1
    with torch_distributed_zero_first(rank):
        dataset = LoadWholeImagesAndLabels(path, imgsz, batch_size,
                                      augment=augment,  # augment images
                                      hyp=hyp,  # augmentation hyperparameters
                                      rect=rect,  # rectangular training
                                      cache_images=cache,
...
class LoadWholeImages:  # for inference
    ...
    def __next__(self):
        ...
        # Padded resize
        img = letterbox(img0, new_shape=w)[0]
...
class LoadWholeImagesAndLabels(Dataset):  # for training/testing
    # 读取测试图片，不做resize读入原图，batchsize只能为1。
    ...
    def __getitem__(self, index):
        ...
        if mosaic:
            ...
            # Load image
            img, (h0, w0), (h, w) = load_whole_image(self, index)

            # Letterbox
            shape = w0  # final letterboxed shape
            img, ratio, pad = letterbox(img, shape, auto=False, scaleup=self.augment)
            shapes = (h0, w0), ((h / h0, w / w0), pad)  # for COCO mAP rescaling
...
# Ancillary functions --------------------------------------------------------------------------------------------------
def load_whole_image(self, index):
    # loads 1 image from dataset, returns img, original hw, resized hw
    img = self.imgs[index]
    if img is None:  # not cached
        path = self.img_files[index]
        ...
        assert img is not None, 'Image Not Found ' + path
        h0, w0 = img.shape[:2]  # orig hw
        return img, (h0, w0), (h0, w0)  # img, hw_original, hw_resized
    else:
        return self.imgs[index], self.img_hw0[index], self.img_hw[index]  # img, hw_original, hw_resized
